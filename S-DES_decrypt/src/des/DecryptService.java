...
public class DecryptService {
    ...
    private boolean[][] cAscii; // 二维布尔数组用于存储每个输入字节的二进制位表示
    private boolean[][] pAscii; // 用于存储解密后的ascii数组
    private boolean[] cBinary; // 输入的二进制串

    // 构造函数
    public DecryptService(String cin, String keyin) {
        // 检查是否为ASCII字符串
        ...
        if (this.isAscii) { // 处理 ASCII 字符串
            ...
            for (int i = 0; i < this.asciiNum; i++) {
                int temp = bytes[i];
                for (int j = 7; j >= 0; j--) {
                    this.cAscii[i][j] = (temp % 2 == 1); // 直接使用布尔值表示
                    temp = temp / 2; // 继续处理下一位
                }
            }
        } else { // 处理二进制串
            int length = cin.length();
            this.cBinary = new boolean[length];
            for (int i = 0; i < length; i++) {
                this.cBinary[i] = (cin.charAt(i) == '1');
            }
        }
    ...
    public String decrypt(String r) { // 解密方法
        StringBuilder outPutPlainText = new StringBuilder(); // 使用 StringBuilder 提高性能
        ...
        if (this.isAscii) { // 处理 ASCII 串
            ...
            for (int i = 0; i < this.asciiNum; i++) {
                this.pAscii[i] = DesFunction(this.cAscii[i], key);
                char thischar = booleanArrayToChar(this.pAscii[i]);
                outPutPlainText.append(thischar);
            }
        } else { // 处理布尔串
            int length = this.cBinary.length;
            for (int i = 0; i < length; i += 8) {
                boolean[] segment = new boolean[Math.min(8, length - i)]; // 处理每 8 位
                System.arraycopy(this.cBinary, i, segment, 0, segment.length);
                boolean[] decryptedSegment = DesFunction(segment, key);
                char asciiList = booleanArrayToChar(decryptedSegment);
                String booleanList = booleanArrayToString(decryptedSegment);
                if(r == "1"){
                    outPutPlainText.append(asciiList);
                } else if (r =="2") {
                    outPutPlainText.append(booleanList);
                }
            }
        }
        return outPutPlainText.toString();
    }
    ...
    //解密函数
    public boolean[] DesFunction(boolean[] cipherText, boolean[] Key) {
        boolean[] cipherText_1 = cipherText;
        boolean[] key_1 = Key;
        boolean[] k1 = kiCreator(key_1,"1");
        boolean[] k2 = kiCreator(key_1,"2");

        cipherText_1 = IP(cipherText_1);
        cipherText_1= f(cipherText_1, k2);
        cipherText_1 = swap(cipherText_1);
        cipherText_1 = f(cipherText_1, k1);
        cipherText_1 = ViseIP(cipherText_1);
        boolean[] plainText = cipherText_1;
        return plainText;
    }
